...
def realign(wavfile, alignment, ms, exp, nthreads=4, progress_cb=None):
    ...
    def realign(chunk):
        ...
        chunk_ms = metasentence.MetaSentence(chunk_transcript, None)
        chunk_ks = chunk_ms.get_kaldi_sequence()

        chunk_gen_hclg_filename = language_model.make_bigram_language_model(chunk_ks, exp)
        k = standard_kaldi.Kaldi(exp, chunk_gen_hclg_filename)
